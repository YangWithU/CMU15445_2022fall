...
class TrieNode {
 public:
   ...
   */
  explicit TrieNode(char key_char) : key_char_(key_char) { this->is_end_ = false; }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    std::unique_ptr<TrieNode> *trverse = &root_;
    std::unique_ptr<TrieNode> *parent{};
    for (auto &x : key) {
      if (trverse->get()->GetChildNode(x) == nullptr) {
        trverse->get()->InsertChildNode(x, std::make_unique<TrieNode>(x));
      }
      parent = trverse;
      trverse = trverse->get()->GetChildNode(x);
    }
    if (trverse->get()->IsEndNode()) {  // is a TrieNodeWithValue
      latch_.WUnlock();
      return false;
    }
    // is a TrieNode
    auto node_with_val = std::make_unique<TrieNodeWithValue<T>>(std::move(**trverse), value);
    parent->get()->RemoveChildNode(key.back());
    parent->get()->InsertChildNode(key.back(), std::move(node_with_val));
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    auto trverse = &root_;
    bool deleted = false;
    auto dfs = [&deleted, &trverse, &key](auto self, size_t depth) -> bool {
      if (depth == key.size()) {
        trverse->get()->SetEndNode(false);
        deleted = true;
        return !trverse->get()->HasChildren();
      }
      if (trverse->get()->GetChildNode(key[depth]) == nullptr) {
        return false;
      }
      trverse = trverse->get()->GetChildNode(key[depth]);
      if (self(self, depth + 1)) {  // 只有在没子结点时才能删，其余只能改is_end
        trverse->get()->RemoveChildNode(key[depth]);
      }
      return !trverse->get()->HasChildren() && !trverse->get()->IsEndNode();
    };
    latch_.WLock();
    dfs(dfs, 0);
    latch_.WUnlock();
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    if (!trverse->get()->IsEndNode()) {
      *success = false;
      latch_.RUnlock();
      return {};
    }
    auto res = dynamic_cast<TrieNodeWithValue<T> *>(trverse->get());
    if (res != nullptr) {
      *success = true;
      latch_.RUnlock();
      return res->GetValue();
